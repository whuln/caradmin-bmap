<template>
  <div class="main">
    ...
    <div class="tablecon" ref="tablecontainer" fill direction="column">
      <mu-data-table
        style="width:100%;"
        selectable
    ...
    <mu-dialog width="360" transition="slide-bottom" fullscreen :open.sync="openAdduserFullscreen">
      <mu-appbar style="height:30px; font-size:12px;" color="red" title="添加用户">
        <mu-button slot="right" flat @click="closeAdduserFullscreenDialog">关闭</mu-button>
      </mu-appbar>
      <div style="padding: 24px;">
        ...
        <mu-button :disabled="locked.add" @click="addUser" color="red">添加</mu-button>
    ...
    <mu-dialog
      ...
      fullscreen
      :open.sync="openUpdateuserFullscreen"
    >
      <mu-appbar style="height:30px" color="red" title="修改用户">
        <mu-button slot="right" flat @click="closeUpdateuserFullscreenDialog">关闭</mu-button>
      </mu-appbar>
      <div style="padding: 24px;">
        <mu-form
          ref="updateuser"
        ...
        <mu-button :disabled="locked.update" @click="updateUser" color="red">修改</mu-button>
      </div>
    </mu-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      accountRules: [
        {
          validate: async val => {
            let resp = await this.r.post("getoneuser", { account: val });
            if (resp.data.code) {
  ...
  methods: {
    ...
    doSearch() {
      ...
      if (this.s_name) {
        this.r
          .post("getusers", { account: this.s_name })
          .then(function(resp) {
            let data = resp.data;
    ...
    updateUser() {
      ...
      // console.log(this.useradd);
      this.r
        .post("updateuser", this.userupdate)
        .then(function(response) {
          // console.log(response.data);
          let data = response.data;
          if (0 == data.code) self.$alert(data.msg);
    ...
    deleteUser() {
      ...
      // console.log(this.useradd);
      this.r
        .post("deleteuser", { account: this.showlist[this.selects[0]].account })
        .then(function(response) {
          // console.log(response.data);
          let data = response.data;
          if (0 == data.code) self.$alert(data.msg);
